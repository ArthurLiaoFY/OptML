...
class Spider:
    def __init__(
        ...
        self.population_value = (
            population_value  # pd.read_json(popuation_value, orient='split').values
        )
        ...
        self.population_weight = [
            self.init_weight for i in range(len(self.population_value))
        ]
    ...
    def mutation(self):
        ...
        self.V = self.X + np.random.uniform(0, 1) * (
            self.population_value[X_end_idx] - self.X
        )
                ...
                if np.random.uniform(0, 1) < self.prob_mut:
                    to_idx = np.random.choice(
                        ...
                        size=np.random.choice(
                            np.append(0, np.arange(2, len(group))), 1
                        )[0],
                        replace=False,
                    )

                    from_idx = np.random.choice(
                        a=to_idx, size=len(to_idx), replace=False
                    )
    ...
    def selection(self):
        cost_values = np.array([self.x2y(self.X), self.x2y(self.V), self.x2y(self.U)])
        ...
        if np.sum(select_mask) == 1:
            ...
            self.X = (
                self.V if argmin_cost == 1 else self.U if argmin_cost == 2 else self.X
            )
        else:
            if argmin_cost == 0:
        ...
        self.population_value = np.insert(
            self.population_value, self.X_init_idx, self.X, axis=0
        )

        self.population_weight = np.insert(
            self.population_weight, self.X_init_idx, self.init_weight + 1, axis=0
        )
    ...
    def run(self, max_iter=None):
        ...
        return (
            self.best_x,
            self.best_y,
            self.udf_func(np.array(self.best_x)[np.newaxis, :])[0],
        )
...
xgb.fit(X, y)
tree_ = DecisionTreeClassifier(min_samples_leaf=50, random_state=config["seed"])
tree_.fit(X, (xgb.predict(X) >= 4).astype(int).astype(str))

spider = Spider(
    cost_func=lambda x: (x[0] - X.mean(axis=0)[0]) ** 2
    + (x[1] - X.mean(axis=0)[1]) ** 2,
    ...
    changeable_group=[
        []
    ],  # np.array([[AOI group idx], [ICT group idx], [UV group idx], ...])
    size_pop=50,
    max_iter=200,
    prob_mut=0.3,
)
...
X_, Y_ = np.meshgrid(
    np.arange(ax0xmin, ax0xmax + ax0xstep, ax0xstep),
    np.arange(ax0ymin, ax0ymax + ax0ystep, ax0ystep),
)
Z_ = f(X_, Y_)
...
ax0.plot(
    *np.array([X.mean(axis=0)[0], X.mean(axis=0)[1], 0]).reshape(-1, 1),
    "r*",
    markersize=18
)
...
ax1xmin, ax1xmax, ax1xstep = (
    X.mean(axis=0)[0] - mean_distance[0] * 1.2,
    X.mean(axis=0)[0] + mean_distance[0] * 1.2,
    0.0005,
)
ax1ymin, ax1ymax, ax1ystep = (
    X.mean(axis=0)[1] - mean_distance[1] * 1.2,
    X.mean(axis=0)[1] + mean_distance[1] * 1.2,
    0.0005,
)

X_, Y_ = np.meshgrid(
    np.arange(ax1xmin, ax1xmax + ax1xstep, ax1xstep),
    np.arange(ax1ymin, ax1ymax + ax1ystep, ax1ystep),
)
Z_ = f(X_, Y_)
...
def animation_func(i):
    ...
    ax2.set_ylim((2, 4))
    ax2.hlines(
        y=X.mean(axis=0)[1], xmin=5.4, xmax=8.2, colors="black", linestyles="dashed"
    )
...
def descent_animation_func(i):
    ax0.plot3D(
        ...
        [
            ...
            spider.cost_func(spider.history_best_X[i + 1]),
        ],
        color="gray",
    )
    ax0.scatter(
